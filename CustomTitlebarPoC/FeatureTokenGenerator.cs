...
namespace CustomTitlebarPoC
{
    public class FeatureTokenGenerator
    {
        ...
        public static string GenerateTokenFromFeatureId(string featureId)
            => GenerateFeatureToken(featureId, LimitedAccessFeaturesMap[featureId], Package.Current.Id.FamilyName);

        public static string GenerateAttestation(string featureId)
            => $"{Package.Current.Id.FamilyName.Split('_').Last()} has registered their use of {featureId} with Microsoft and agrees to the terms of use.";
